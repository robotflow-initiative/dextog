...
class DrinkCallback(BaseCallback):
    """
...
class DrinkChildEnv:
    ...
    def reset_wait(self):
        ...
        joint_upper_limit = 6.28
        joint_lower_limit = 0.0
        object_dof = self.object.data["joint_positions"][0]
        self.obj_initial_joint_normalized = (np.clip(object_dof, joint_lower_limit, joint_upper_limit)
    ...
    def _get_obs(self):
        ...
        cap_positions = np.array(self.object.data["positions"])[[2]]
        ...
        return {
            "cap_position": cap_positions.mean(axis=0) - object_positions.mean(axis=0),
    ...
    def _get_reward(self):
        ...
        joint_upper_limit = 6.28
        joint_lower_limit = 0.0
        object_dof_normalized = (np.clip(object_dof, joint_lower_limit, joint_upper_limit)
                                 / (joint_upper_limit - joint_lower_limit)
                                 )

        obj_joint_delta_normalized = object_dof_normalized - self.obj_initial_joint_normalized
        goal_reward = 80 * obj_joint_delta_normalized

        fingertip_positions = np.array(self.shadow.data["positions"])[
            np.array([8, 14, 19, 24, 29]) - 1
            ]
        cap_position = np.array(self.object.data["positions"])[2]
        avg_fingertip_distance = np.mean(
            np.linalg.norm(fingertip_positions - cap_position, axis=1)
        )

        success = obj_joint_delta_normalized > 0.6
        success_reward = 50.0 if success else 0

        reward = goal_reward + success_reward - avg_fingertip_distance
    ...
    def step_async(self, action):
        ...
        self.rfu.SendObject("WorldMove", self.id, 0., 0., 0.)
...
class RFUVecEnv(DummyVecEnv):
    def __init__(
        ...
        self.rfu = RFUniverseBaseEnv(
            executable_file=osp.join(osp.split(__file__)[0], "../assets/envs/drink/drink_env.x86_64"),
...
def DrinkVecEnv(num_envs, proc_id=0, shared_data=None, **kwargs):
    import os
