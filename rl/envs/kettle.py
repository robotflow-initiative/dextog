...
class KettleCallback(BaseCallback):
    """
...
class KettleChildEnv:
    ...
    def reset_wait(self):
        ...
        self.obj_initial_joint_normalized = (self.object.data["joint_upper_limit"][0] - self.object.data["joint_positions"][0]
                                                    ) / (
    ...
    def _get_obs(self):
        ...
        object_positions = np.array(self.object.data["positions"])[[1, 2]]
        button_positions = np.array(self.object.data["positions"])[[4]]
    ...
    def step_async(self, action):
        ...
        # self.rfu.SendObject("WorldMove", self.id, 0.0, base_action[1], 0.0)
...
class RFUVecEnv(DummyVecEnv):
    def __init__(
        ...
        self.rfu = RFUniverseBaseEnv(
            executable_file=osp.join(osp.split(__file__)[0], "../assets/envs/kettle/kettle_env.x86_64"),
...
def KettleVecEnv(num_envs, proc_id=0, shared_data=None, **kwargs):
    import os
