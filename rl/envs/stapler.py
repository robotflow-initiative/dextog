...
class StaplerCallback(BaseCallback):
    """
...
class StaplerChildEnv:
    ...
    def reset_wait(self):
        ...
        self.obj_initial_joint_normalized = (
            self.object.data["joint_upper_limit"][0] - self.object.data["joint_positions"][0]
        ) / (
    ...
    def _get_obs(self):
        ...
        object_positions = np.array(self.object.data["positions"])[[1, 2, 4]]
        ...
        hand_position = np.clip(np.asarray(self.shadow.data['positions'][29] - self.shadow_initial_position), -1, 1)
        hand_object_offset = np.clip(shadow_positions - object_positions.mean(axis=0), -1, 1)
        return {
            "hand_position": hand_position,
            "hand_object_offset": hand_object_offset,
    ...
    def _get_reward(self):
        ...
        object_dof_normalized = (
            self.object.data["joint_upper_limit"][0] - object_dof
        ) / (
            self.object.data["joint_upper_limit"][0]
            - self.object.data["joint_lower_limit"][0]
        )
        ...
        hand_object_dist = (np.linalg.norm(np.asarray(self.object.data['positions'][0]) - np.asarray(self.shadow.data['positions'][29])))
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rfu = RFUniverseBaseEnv(
            executable_file=osp.join(osp.split(__file__)[0], "../assets/envs/stapler/stapler_env.x86_64"),
...
def StaplerVecEnv(num_envs, proc_id=0, shared_data=None, **kwargs):
    import os
